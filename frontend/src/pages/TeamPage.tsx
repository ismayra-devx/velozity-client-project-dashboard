  ...
  const loadData = async () => {
    try {
      ...
      if (currentUser?.role === 'PROJECT_MANAGER') {
        // PM sees their developers/team rather than entire agency
        const devMap = new Map<string, User>();
        taskList.forEach((t) => {
          if (t.assignedTo) {
        ...
        setTeamMembers(Array.from(devMap.values()));
      } else {
        // Admin: collect all distinct members (Admin, PMs, Developers)
        const userMap = new Map<string, User>();
        if (currentUser) userMap.set(currentUser.id, currentUser);
        projectList.forEach((p) => {
          if (p.createdBy) userMap.set(p.createdBy.id, p.createdBy);
        });
        developers.forEach((d) => userMap.set(d.id, d));
    ...
    <div className="space-y-6 pb-12">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-200/80 pb-5">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-1">
            {isPM
              ? 'Developers assigned to tasks across your active projects.'
      ...
      <div className="bg-white border border-slate-200/80 rounded-xl p-4 flex flex-col sm:flex-row items-center justify-between gap-4">
        <div className="relative w-full sm:w-80">
          ...
          <input
            type="text"
            placeholder={isPM ? "Search developers..." : "Search team members..."}
            value={searchQuery}
      ...
      {loading ? (
        ...
        <div className="dashboard-card p-16 text-center text-slate-500">
          ...
          <p className="text-sm font-semibold text-slate-800">
            {isPM ? 'No assigned developers found' : 'No team members found'}
          </p>
          <p className="text-xs text-slate-500 mt-1">
            {isPM
              ? 'Developers will appear here once tasks in your projects are assigned to them.'
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {filteredMembers.map((member) => {
            ...
            return (
              <div
                key={member.id}
                className="dashboard-card p-5 hover:border-slate-300 transition-all flex flex-col justify-between"
              >
                <div>
                  ...
                  <div className="text-xs font-semibold text-slate-700 mb-3">
                    {userTasks.length} {userTasks.length === 1 ? 'assigned task' : 'assigned tasks'}
                  </div>
                </div>
                ...
                <div className="pt-3 border-t border-slate-100 flex items-center justify-between text-xs gap-2">
                  ...
                  <button
                    ...
                    className="text-slate-500 hover:text-slate-800 text-[11px] font-medium px-2 py-1 rounded bg-slate-50 hover:bg-slate-100 transition-colors"
                  >
                    View workload
                  </button>
                </div>
